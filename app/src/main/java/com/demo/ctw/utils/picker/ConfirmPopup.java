...
public abstract class ConfirmPopup<V extends View> extends BottomPopup<View> implements View.OnClickListener {
    ...
    private boolean topTitleVisible = false;
    private boolean isJudge = false;        //判断是否验证
    private int topLineColor = 0xFFDDDDDD;
    private int topBackgroundColor = Color.WHITE;
    private boolean cancelVisible = true;
    private CharSequence cancelText = "", submitText = "", titleText = "";
    ...
    //设置顶部标题是否显示
    public void setTopTitleVisible(boolean topLineVisible) {
        this.topTitleVisible = topLineVisible;
    }

    public void setTopTitleText(CharSequence titleText) {
    ...
    protected View getView() {
        ...
        topButtonLayout.setLayoutParams(new RelativeLayout.LayoutParams(MATCH_PARENT, CompatUtils.toPx(activity, 40)));
        ...
        if (topTitleVisible || !TextUtils.isEmpty(titleText)) {
            ...
            topButtonLayout.addView(title);
            topButtonLayout.setLayoutParams(new RelativeLayout.LayoutParams(MATCH_PARENT, CompatUtils.toPx(activity, 60)));
        }
    ...
    public void onClick(View v) {
        ...
        if (!isJudge && isShowing())   //不需要验证则直接消失
            dismiss();
    }
