...
public class AddressChoosePopDialog {
    ...
    private AddressChoosePopDialog() {
     ...
     * 显示在控件的下方
     */
    public AddressChoosePopDialog showDownPop(View parent) {
     ...
     * 显示在控件的上方
     */
    public AddressChoosePopDialog showAsUp(View view) {
...
//                mPopupWindow.showAsDropDown(view, 0, -(getHeight() + view.getMeasuredHeight()));
     ...
     * 显示在控件的左边
     */
    public AddressChoosePopDialog showAsLeft(View view) {
        if (view.getVisibility() == View.GONE) {
            mPopupWindow.showAtLocation(view, Gravity.NO_GRAVITY, 0, 0);
        } else {
            int[] location = new int[2];
     ...
     * 显示在控件右边
     */
    public AddressChoosePopDialog showAsRight(View view) {
        if (view.getVisibility() == View.GONE) {
            mPopupWindow.showAtLocation(view, Gravity.NO_GRAVITY, 0, 0);
        } else {
            int[] location = new int[2];
     ...
     * @return
     */
    public AddressChoosePopDialog showAsDown(View view) {
        if (mPopupWindow != null) {
     ...
     * 全屏弹出
     */
    public AddressChoosePopDialog showAsBottom(View view) {
        if (view.getVisibility() == View.GONE) {
            mPopupWindow.showAtLocation(view, Gravity.NO_GRAVITY, 0, 0);
        } else {
            int[] location = new int[2];
    ...
    public AddressChoosePopDialog showAtLocation(View anchor, int gravity, int x, int y) {
        if (mPopupWindow != null) {
    ...
    public static class Builder implements PopupWindow.OnDismissListener {
        ...
        public AddressChoosePopDialog build(Context context) {
            this.mContext = context;
            AddressChoosePopDialog popWindow = new AddressChoosePopDialog();
            apply();
